// Code generated by protoc-gen-tarp. DO NOT EDIT. client_v2_rpc/rpc
...
import 'rpc.pb.dart';
import '../client_v2_rpc/channel.pb.dart';


// ClientRPC specific RPC methods.// for end-user interactions.
abstract class ClientRPC {
  // Get or create a channel.// If the channel does not exist, it will be created. but if it does, it will be returned.
  Future<GetChannelResponse> getChannel(twirp.Context ctx, GetChannelRequest req);
  // Query channels.// Returns a list of channels that match the query parameters.// TODO: rename to QueryChannels
  Future<QueryChannelsResponse> queryChannels(twirp.Context ctx, QueryChannelsRequest req);
}
...
class ClientRPCJSONClient implements ClientRPC {
  ...
  @override
  Future<GetChannelResponse> getChannel(twirp.Context ctx, GetChannelRequest req) async {
    ctx = twirp.withPackageName(ctx, 'client_v2_rpc');
    ctx = twirp.withServiceName(ctx, 'ClientRPC');
    ctx = twirp.withMethodName(ctx, 'GetChannel');
    return interceptor((ctx, req) {
      return callGetChannel(ctx, req);
    })(ctx, req);
  }

  Future<GetChannelResponse> callGetChannel(twirp.Context ctx, GetChannelRequest req) async {
    try {
      Uri url = Uri.parse(baseUrl + prefix + 'stream.chat.client_v2_rpc.ClientRPC/GetChannel');
      final data = await doJSONRequest(ctx, url, hooks, req);
      final GetChannelResponse res = GetChannelResponse.create();
  ...
  @override
  Future<QueryChannelsResponse> queryChannels(twirp.Context ctx, QueryChannelsRequest req) async {
    ctx = twirp.withPackageName(ctx, 'client_v2_rpc');
    ctx = twirp.withServiceName(ctx, 'ClientRPC');
    ctx = twirp.withMethodName(ctx, 'QueryChannels');
    return interceptor((ctx, req) {
      return callQueryChannels(ctx, req);
    })(ctx, req);
  }

  Future<QueryChannelsResponse> callQueryChannels(twirp.Context ctx, QueryChannelsRequest req) async {
    try {
      Uri url = Uri.parse(baseUrl + prefix + 'stream.chat.client_v2_rpc.ClientRPC/QueryChannels');
      final data = await doJSONRequest(ctx, url, hooks, req);
      final QueryChannelsResponse res = QueryChannelsResponse.create();
      res.mergeFromProto3Json(json.decode(data));
      return Future.value(res);
    } catch (e) {
      rethrow;
